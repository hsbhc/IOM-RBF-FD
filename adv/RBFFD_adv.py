...
def alpha(points, c):
    ...
    return torch.mm(torch.inverse(A), p1 - b)
...
def epsilon(points, points_u, c):
    ...
    ll_equation = torch.autograd.grad(l_equation, center, grad_outputs=torch.ones_like(l_equation), create_graph=True)[
        0]
    return l_equation - ll_equation


def U(x):
    return torch.sin(torch.pi * x) + (torch.exp(x) - 1) / (torch.e - 1)


def F(x):
    return torch.pi ** 2 * torch.sin(torch.pi * x) + torch.pi * torch.cos(torch.pi * x)
...
if __name__ == '__main__':
    ...
    # # 记录基函数在不同节点个数时的性能
    # file = open('RBFFD_adv_info.txt', 'w+')
